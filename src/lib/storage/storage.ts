...
export function loadState(): AppState {
  try {
    ...
    if (!raw) {
      localStorage.setItem(DUMMY_RESET_KEY, "1"); // seed is already clean, no future wipe needed
      return SEED_STATE;
    }
    const parsed = JSON.parse(raw) as AppState;

    // One-time migration: savings deposits, goals, and bank accounts were dummy
    // demo data in the local pre-release environments. Clear them once so the
    // user starts these from scratch on first production release. Bills,
    // plans, and income are left intact. Since there are no existing production
    // users, a blanket clear is safe and intended for this release.
